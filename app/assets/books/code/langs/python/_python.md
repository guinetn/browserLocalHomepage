# Python

[Packages pypi](https://pypi.org)
>pip3 install <package_name>


Python is nothing more than a bunch of rules.    
The rules that govern Python could be implemented in some languages.     
The most popular Python implementation is in C.    
- https://github.com/python/cpython

On top of that, a decent number of Python packages like NumPy are written with C and its younger brother C++ to address efficiency drawbacks because Python is dead slow in comparison.

Cython creates C modules that speed up Python code execution, important for complex applications where an interpreted language isn't efficient.

## in vs code
- [VSCode's Python Interactive mode (= notebook live) is AMAZING!](https://www.youtube.com/watch?v=lwN4-W1WR84&t=358s)

## Python to .exe
download.page(code/langs/python/python_to_exe.md)

## Great projects
- [Computer Vision Security Camera + Alarm](https://www.youtube.com/watch?v=UOIKXp82iEw)
- [Computer Vision to Control Apps (~sendkeys)](https://github.com/ProgrammingHero1/webcamfun)
- [Alexa in Python](https://www.youtube.com/watch?v=AWvsXxDtEkU)
- https://towardsdatascience.com/build-dashboards-in-less-than-10-lines-of-code-835e9abeae4b

## Tools
- https://towardsdatascience.com/rich-generate-rich-and-beautiful-text-in-the-terminal-with-python-541f39abf32e


## More
- https://pythonprogramming.net ***
- https://towardsdatascience.com/the-perfect-python-cheatsheet-for-beginners-c7c16799de5

download.page(code/langs/python/python.md)
download.page(code/langs/python/ipython.md)
download.page(code/langs/python/pep.md)

download.page(code/langs/python/pip.md)
download.page(code/langs/python/pip_make_packages.md)

## PERFORMANCES: EXTEND PYTHON (C C++ C#)#

download.code(assets/books/code/langs/python/perfs_extend_to_c_c++/_perfs_extend_to_c_c++.md)

## ASSERT
download.code(assets/books/code/langs/python/code/assert.py)
## * (ASTERISK): UNPACKING
download.code(assets/books/code/langs/python/code/asterisk_unpacking.py)
## ASYNC
download.code(assets/books/code/langs/python/code/async.py)
## BUILT-IN-FUNCTIONS
download.code(assets/books/code/langs/python/code/built-in-functions.py)

## CLASS
download.code(assets/books/code/langs/python/code/class.py)
## CLASS: MAGIC METHODS
download.code(assets/books/code/langs/python/code/class_magic_methods.py)
## COLLECTIONS
download.code(assets/books/code/langs/python/code/coll.py)
## COLL DEQUE
download.code(assets/books/code/langs/python/code/coll_deque.py)
## COLL DICTIONARIES DEFAULT-DICT COUNTER
download.code(assets/books/code/langs/python/code/coll_dictionaries_defaultdict_counter.py)
## COLL GENERATORS
download.code(assets/books/code/langs/python/code/coll_generators.py)
## COLL GRAPHS
download.code(assets/books/code/langs/python/code/coll_graphs.py)
## COLL HASHTABLES
download.code(assets/books/code/langs/python/code/coll_hashtables.py)
## COLL LINKEDLIST
download.code(assets/books/code/langs/python/code/coll_linkedlist.py)
## COLL LISTS ARRAYS
download.code(assets/books/code/langs/python/code/coll_lists-arrays.py)
## COLL LISTS COMPREHENSION
download.code(assets/books/code/langs/python/code/coll_lists_comprehension.py)
## COLL RANGE
download.code(assets/books/code/langs/python/code/coll_range.py)
## COLL SETS
download.code(assets/books/code/langs/python/code/coll_sets.py)
## COLL STACKS
download.code(assets/books/code/langs/python/code/coll_stacks.py)
## COLL TREES
download.code(assets/books/code/langs/python/code/coll_trees.py)
## COLL TUPLES
download.code(assets/books/code/langs/python/code/coll_tuples.py)
## COLL ZIP
download.code(assets/books/code/langs/python/code/coll_zip.py)
## COMMENTS
download.code(assets/books/code/langs/python/code/comments.py)
## CONDITIONALS
download.code(assets/books/code/langs/python/code/conditionals.py)
## CONTROL FLOW LOOPS
download.code(assets/books/code/langs/python/code/control_flow_loops.py)
download.code(assets/books/code/langs/python/code/control_flow_itertools.py)
## CONTROL FLOW SWITCH
download.code(assets/books/code/langs/python/code/control_flow_switch.py)
## CSV
download.code(assets/books/code/langs/python/code/csv.py)
## DB SQLITE3
download.code(assets/books/code/langs/python/code/db_sqlite3.py)
## DEBUG
download.code(assets/books/code/langs/python/code/debug.py)
## DEBUG - LOG
download.code(assets/books/code/langs/python/code/debug-log.py)
## DEBUG VS CODE
download.code(assets/books/code/langs/python/code/debug_vs_code.py)
## DOCSTRINGS
download.code(assets/books/code/langs/python/code/docstrings.py)
## EXCEL
download.code(assets/books/code/langs/python/code/excel.py)
## EXCEPTIONS
download.code(assets/books/code/langs/python/code/exceptions.py)
## FILES
download.code(assets/books/code/langs/python/code/files.py)
## FILES COPY
download.code(assets/books/code/langs/python/code/files_copy.py)
## FILES RENAME INSIDE
download.code(assets/books/code/langs/python/code/files_rename_inside.py)
## FILES ZIP
download.code(assets/books/code/langs/python/code/files_zip.py)
## FILE CSV
download.code(assets/books/code/langs/python/code/files_csv.py)

## FUNCTIONAL PROGRAMMING PYTHON
download.code(assets/books/code/langs/python/code/functional_programming_Python.py)
## CLASSIC
download.code(assets/books/code/langs/python/code/functions-classic.py)
## FUNCTIONS
download.code(assets/books/code/langs/python/code/functions.py)
## FUNCTIONS - FUNCTOOLS
download.code(assets/books/code/langs/python/code/functions_functools.py)
## FUNCTIONS CLOSURE
download.code(assets/books/code/langs/python/code/functions_closure.py)
## FUNCTIONS DECORATORS
download.code(assets/books/code/langs/python/code/functions_decorators.py)
## REDUCE
download.code(assets/books/code/langs/python/code/functions_fmr-reduce.py)
## FUNCTIONS FMR FILTER
download.code(assets/books/code/langs/python/code/functions_fmr_filter.py)
## FUNCTIONS FMR MAP
download.code(assets/books/code/langs/python/code/functions_fmr_map.py)
## FUNCTIONS LAMBDA
download.code(assets/books/code/langs/python/code/functions_lambda.py)
## FUNCTION ANNOTATIONS
download.code(assets/books/code/langs/python/code/function_annotations.py)
## MAHOTAS
download.code(assets/books/code/langs/python/code/images-mahotas.py)
## NUMPY
download.code(assets/books/code/langs/python/code/images-numpy.py)
## OPENCV
download.code(assets/books/code/langs/python/code/images-opencv.py)
## PGMAGICK
download.code(assets/books/code/langs/python/code/images-pgmagick.py)
## PILLOW
download.code(assets/books/code/langs/python/code/images-pillow.py)
## SCIPY
download.code(assets/books/code/langs/python/code/images-scipy.py)
## SIMPLECV
download.code(assets/books/code/langs/python/code/images-simplecv.py)

download.code(assets/books/code/langs/python/code/images_bg_added.jpg)
## INPUTS
download.code(assets/books/code/langs/python/code/inputs.py)

# LIBS

## LIBS BCRYPT
download.code(assets/books/code/langs/python/code/libs_bcrypt.py)

## LIBS BEAUTIFULSOUP
download.code(assets/books/code/langs/python/code/libs_web_beautifulsoup.py)
## LIBS SCRAPY
download.code(assets/books/code/langs/python/code/libs_web_scrapy.py)

## FAKER: fake data
download.code(assets/books/code/langs/python/code/libs_faker.py)

## LIBS DASH
download.code(assets/books/code/langs/python/code/libs_dash.py)
## LIBS DATEUTIL
download.code(assets/books/code/langs/python/code/libs_dateutil.py)
## LIBS FLASK
download.code(assets/books/code/langs/python/code/libs_web_flask.py)
## LIBS GLOB
download.code(assets/books/code/langs/python/code/libs_glob.py)
## LIBS IMAGES FACE DETECTION  XALIENT
download.code(assets/books/code/langs/python/code/libs_images_face_detection__xalient.py)
## LIBS INSPECT
download.code(assets/books/code/langs/python/code/libs_inspect.py)
## LIBS IPYTHON
download.code(assets/books/code/langs/python/code/libs_ipython.py)
## LIBS IPYWIDGETS
download.code(assets/books/code/langs/python/code/libs_ipywidgets.py)
## LIBS MULTIPROCESSING
download.code(assets/books/code/langs/python/code/libs_multiprocessing.py)
## LIBS NUMPY
download.code(assets/books/code/langs/python/code/libs_numpy.py)
## LIBS OPENCV
download.code(assets/books/code/langs/python/code/libs_images_opencv.py)
## LIBS OS
download.code(assets/books/code/langs/python/code/libs_os.py)
## LIBS PILLOW
download.code(assets/books/code/langs/python/code/libs_images_pillow.py)
## LIBS PROGRESSBAR
download.code(assets/books/code/langs/python/code/libs_progressbar.py)
## LIBS PYGAME
download.code(assets/books/code/langs/python/code/libs_PyGame.py)
## LIBS REQUESTS
download.code(assets/books/code/langs/python/code/libs_requests.py)
## LIBS SERIALIZATION  PICKLE
download.code(assets/books/code/langs/python/code/libs_serialization_pickle.py)
## LIBS STREAMLIT
download.code(assets/books/code/langs/python/code/libs_streamlit.py)
## LIBS SUBPROCESS
download.code(assets/books/code/langs/python/code/libs_subprocess.py)
## LIBS TABULATE
download.code(assets/books/code/langs/python/code/libs_tabulate.py)
## LIBS TQDM
download.code(assets/books/code/langs/python/code/libs_tqdm.py)
## MAIN
download.code(assets/books/code/langs/python/code/main.py)
## STATISTICS
download.code(assets/books/code/langs/python/code/maths-statistics.py)
## MATHS
download.code(assets/books/code/langs/python/code/maths.py)
## MATH RANDOM
download.code(assets/books/code/langs/python/code/math_random.py)
## APOLLO13
download.code(assets/books/code/langs/python/code/matplotlib_euler-apollo13.py)
## MATPLOTLIB SIN
download.code(assets/books/code/langs/python/code/matplotlib_sin.py)
## MATPLOTLIB YEAR VALUES
download.code(assets/books/code/langs/python/code/matplotlib_year_values.py)
## MATRIX
download.code(assets/books/code/langs/python/code/matrix.py)
## MICROSERVICES
download.code(assets/books/code/langs/python/code/microservices.py)
## MODULES
download.code(assets/books/code/langs/python/code/modules.py)
## MODULES IMPORTS
download.code(assets/books/code/langs/python/code/modules_imports.py)
## MODULES PACKAGES
download.code(assets/books/code/langs/python/code/modules_packages.py)
## TEXT
download.code(assets/books/code/langs/python/code/pdf-extract-text.py)
## PIP VIRTUALENV
download.code(assets/books/code/langs/python/code/pip_virtualenv.py)
## RASPBERRYPI
download.code(assets/books/code/langs/python/code/raspberryPi.py)
## REGEX
download.code(assets/books/code/langs/python/code/regex.py)
## SNIPPETS
download.code(assets/books/code/langs/python/code/snippets.py)
## SORTING
download.code(assets/books/code/langs/python/code/sorting.py)
## AUDIOBOOK
download.code(assets/books/code/langs/python/code/sounds-audiobook.py)
## SPEAK
download.code(assets/books/code/langs/python/code/sounds-tts-speak.py)
## SOUNDS
download.code(assets/books/code/langs/python/code/sounds.py)
## THREADING
download.code(assets/books/code/langs/python/code/threading.py)
## TYPES
download.code(assets/books/code/langs/python/code/types.py)
## TYPES BOOL
download.code(assets/books/code/langs/python/code/types_bool.py)
## TYPES DATES
download.code(assets/books/code/langs/python/code/types_dates.py)
## TYPES NUMBERS
download.code(assets/books/code/langs/python/code/types_numbers.py)
## TYPES STRINGS
download.code(assets/books/code/langs/python/code/types_strings.py)
## TYPES TIME
download.code(assets/books/code/langs/python/code/types_time.py)
## TYPES TUPLES SETS
download.code(assets/books/code/langs/python/code/types_tuples_sets.py)
## INPUT
download.code(assets/books/code/langs/python/code/user_inputs.py)
## VALIDATOR
download.code(assets/books/code/langs/python/code/validator.py)
## VARIABLES
download.code(assets/books/code/langs/python/code/variables.py)
download.code(assets/books/code/langs/python/code/variables_env_vars.py)
## WALRUS OPERATOR
download.code(assets/books/code/langs/python/code/walrus_operator,.py)


## JSON
download.code(assets/books/code/langs/python/code/web_json.py)

## WEB
download.code(assets/books/code/langs/python/code/web.py)
### FASTAPI
download.code(assets/books/code/langs/python/code/web_fastapi.py)
## API
download.code(assets/books/code/langs/python/code/api.py)
## FTP
download.code(assets/books/code/langs/python/code/web_ftp.py)

download.code(assets/books/code/langs/python/code/doctest_test_interactive_python_examples.py)
download.code(assets/books/code/langs/python/code/files_scripting.py)
download.code(assets/books/code/langs/python/code/functions_fastcore.py)


## DOCUMENTS: pdf...

download.code(assets/books/code/langs/python/code/libs_export_pdf.py)
download.code(assets/books/code/langs/python/code/libs_office_word_excel.py)

