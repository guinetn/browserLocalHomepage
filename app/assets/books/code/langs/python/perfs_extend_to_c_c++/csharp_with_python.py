...
Calling Python Script Using New Process Initialization.
 ...
 such code you need to consider permissions of the account under which your main application is running. For example, if you want to run a script from the web service, that is hosted on IIS — be sure that all the script files that need to be executed have the right permissions for the user under which your web service’s ApplicationPool is running.
...
You can host the IronPython interpreter right in your .NET application. For example, using NuGet,
