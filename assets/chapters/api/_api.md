# API

::::
download.chapter(api/rest.md)

download.chapter(api/soap.md)
::::
download.chapter(api/grpc.md)
::::
download.chapter(api/performances.md)
::::
download.chapter(api/cors.md)
::::    
download.chapter(web/back/api/node.md)
:::: 
download.chapter(web/back/api/express.md)
::::
download.chapter(web/back/api/flask.md)
:::: 
download.chapter(web/back/api/aspnet.md)
::::
download.chapter(web/back/api/aspnetcore.md)
::::
download.chapter(web/back/api/open_api.md)
::::
download.chapter(web/back/health_check.md)
::::
download.chapter(api/testing/_testing.md)
::::

## Open APIS

- https://github.com/public-apis/public-apis

## More

URL shortening service like bit.ly or goo.gl
- https://github.com/abhinavdhasmana/tinyUrl
- https://medium.com/@adhasmana/system-design-create-a-url-shortening-service-part-1-overview-26aae5597914
- https://github.com/abhinavdhasmana/ratelimiter
- https://www.ezzylearning.net/tutorial/a-developers-guide-for-creating-web-apis-with-asp-net-core-5
- https://auth0.com/blog/how-to-build-and-secure-web-apis-with-aspnet-core-3/
- https://www.tutorialsteacher.com/webapi/consuming-web-api-in-dotnet-using-httpclient
- https://www.ezzylearning.net/tutorial/a-developers-guide-for-creating-web-apis-with-asp-net-core-5